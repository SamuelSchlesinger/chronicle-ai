...
use bevy::prelude::*;
use bevy_egui::egui;
use chronicler_core::character_builder::point_buy_cost;
use chronicler_core::spells::{get_spell, spells_by_level, SpellClass};
...
use chronicler_core::{AbilityMethod, CharacterBuilder};
            ...
            match *method {
                ...
                AbilityMethod::Rolled => {
                    creation.rolled_scores =
                        chronicler_core::character_builder::roll_ability_scores();
                    creation.ability_scores = AbilityScores::default();
                    creation.ability_assignment = [None; 6];
                }
            }
...
fn render_rolled(ui: &mut egui::Ui, creation: &mut CharacterCreation) {
    ...
    ui.horizontal(|ui| {
        ...
        if ui.button("Re-roll").clicked() {
            creation.rolled_scores = chronicler_core::character_builder::roll_ability_scores();
            creation.ability_assignment = [None; 6];
            creation.ability_scores = AbilityScores::default();
        }
    });
    ...
    ui.horizontal(|ui| {
        if ui.button("Save Character").clicked() {
            ...
            match creation.build_character() {
                Ok(character) => {
                    let saved = chronicler_core::SavedCharacter::new(character);
                    let path = chronicler_core::persist::character_save_path(
                        &characters_path,
                        &creation.name,
                    );
