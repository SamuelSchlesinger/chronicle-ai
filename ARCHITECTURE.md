...
| Constraint | Decision |
|------------|----------|
| Primary goal | D&D Dungeon Master game |
...
│       └── ...
├── dnd-core/            # Game logic + AI DM (the heart)
│   └── src/
...
| Current | Why Remove |
|---------|------------|
| `Agent` trait | D&D has one agent type (DM); generic trait adds boilerplate with no benefit |
| `SafetyValidator`, `Guardrail` | D&D doesn't need safety validation pipelines |
| Generic `Memory` traits | Specialized DmMemory for D&D needs; generic traits unused |
...
| `AgentId`, `SessionId`, etc. newtypes | Useful, but D&D only needs `CharacterId`, `LocationId`, etc. |
...
2. Create `dnd-core/` crate with `GameSession` API
