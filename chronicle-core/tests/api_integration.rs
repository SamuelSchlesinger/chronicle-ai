...
//! Run with: `cargo test -p chronicle-core --test api_integration -- --ignored`
//!
...
use chronicle_core::dm::story_memory::ConsequenceSeverity;
...
fn create_sample_wizard(name: &str) -> Character {
    ...
    character.race = Race {
        ...
        subrace: None,
        race_type: Some(chronicle_core::world::RaceType::Human),
    };
...
fn add_hostile_npc(world: &mut GameWorld, name: &str, description: &str) {
    ...
    npc.disposition = chronicle_core::world::Disposition::Hostile;
...
#[tokio::test]
#[ignore] // Run with: cargo test -p chronicle-core --test api_integration -- --ignored
...
async fn test_dm_handles_attack_roll() {
    ...
    let has_attack_intent = response
        .intents
        .iter()
        .any(|i| matches!(i, chronicle_core::rules::Intent::Attack { .. }));
...
async fn test_dm_handles_skill_check() {
    ...
    let has_skill_check = response.intents.iter().any(|i| {
        matches!(
            i,
            chronicle_core::rules::Intent::SkillCheck { .. }
                | chronicle_core::rules::Intent::AbilityCheck { .. }
        )
    });
...
async fn test_dm_handles_short_rest() {
    ...
    let has_rest_intent = response
        .intents
        .iter()
        .any(|i| matches!(i, chronicle_core::rules::Intent::ShortRest));
...
async fn test_dm_handles_long_rest_spell_recovery() {
    ...
    let has_rest_intent = response
        .intents
        .iter()
        .any(|i| matches!(i, chronicle_core::rules::Intent::LongRest));
